...
@section('content')
    <style>
        .map-wrapper { position: relative; margin: -40px -40px 0 -40px; }
        #mainMap { width: 100%; height: calc(100vh - 73px); }
        ...
        .map-filter-panel h3 { font-size: 1rem; font-weight: 700; color: #1a1a1a; margin: 0; }
        .filter-clear { font-size: 0.75rem; font-weight: 600; color: var(--maroon, #6B1D2A); cursor: pointer; text-decoration: none; }
        .filter-section-label { font-size: 0.6rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: #9ca3af; margin: 16px 0 10px; }

        .filter-checkbox { display: flex; align-items: center; gap: 10px; padding: 6px 0; cursor: pointer; }
        .filter-checkbox input[type="checkbox"] { width: 18px; height: 18px; accent-color: var(--maroon, #6B1D2A); cursor: pointer; }
        .filter-dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; }
        .filter-checkbox span { font-size: 0.85rem; color: #4b5563; }
        ...
        .legend-item { display: flex; align-items: center; gap: 6px; font-size: 0.68rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; color: #4b5563; }

        .map-search {
            position: absolute; top: 16px; left: 260px; right: 160px; z-index: 1000;
        }
        .map-search input {
            width: 100%; max-width: 420px; padding: 12px 16px 12px 42px;
            border-radius: 12px; border: 1px solid rgba(107,29,42,0.08);
            background: rgba(255,255,255,0.95); backdrop-filter: blur(8px);
            font-size: 0.85rem; color: #1a1a1a; outline: none;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // ===== Initialize Map =====
        const map = L.map('mainMap', { zoomControl: false }).setView([-6.2088, 106.8456], 12);
